...
name: arch-guide
description: >
  消费 arch-baseline 产出的 `01-architecture.md` 业务域地图,结合 CodeGraph + GitNexus 对核心业务域
  逐个**深描业务主线**,产出供**人**学习/交接/介绍的完整架构导览(目标仓 `.brownfield/architecture/`)。
  与 arch-baseline 是上下游:arch-baseline 产薄地图(给带 MCP 的消费方),本 skill 产厚导览(给人)。
...
| | arch-baseline | arch-guide(本 skill) |
|---|---|---|
| 消费者 | 带 MCP 的 agent(下游消费方) | **人**(学习/交接/介绍) |
...
- 目标仓已建双索引,且**通过前置完整性闸门**——复用 [`../arch-baseline-skill/references/preflight-check.md`](../arch-baseline-skill/references/preflight-check.md);
  BLOCK 则停止、CONFIRM 则人工确认后继续,**绝不在残缺索引上生成导览**。
- 目标仓已存在 **人审过的** `.brownfield/01-architecture.md`(本 skill 的输入目录);若无,先跑 arch-baseline。

## 不变量(核心纪律)

0. **前置闸门 + 输入校验(强制·首步)**:复用 arch-baseline 前置闸门确认双索引完整;并校验 `01-architecture.md`
...
5. **复用不重造**:每条主流程用 6 步重建法 [`../arch-baseline-skill/references/reconstruction-method.md`](../arch-baseline-skill/references/reconstruction-method.md);
...
- **G0 前置(强制)** ← 复用 arch-baseline 闸门 + 校验 01 存在/已审 → 不满足则停止。
...
- **上游**:依赖同域 skill `arch-baseline` 的 `01` 地图、前置闸门与 6 步法。
- **下游**:产物 `.brownfield/architecture/` 是独立产物,可被任意下游(文档站、知识库、规约/交付流水线等)消费;
  本 wf **不感知、不依赖**任何特定下游。
- 本 skill 不修改目标仓源码。
