...
> **定名**:critic 是「**默认启用、可显式豁免的软门**」(workflow 在阶段切换前必读其 verdict;用户在 chat 中 `/critic skip` 可显式豁免)。本文件及所有协作文档统一使用「软门」一词,**不再混用**「软审查」/「软审查门」/「建议检查」等同义异称。
...
| 与 audit | critic 是「软门」(LLM-as-Judge 语义层),audit 是「硬门」(deterministic scripts);两者**正交叠加** |
...
6. **显式豁免**:critic 是「软门」,默认启用;用户可显式豁免(在 chat 中输入 `/critic skip`,workflow 据此跳过本轮转移前检查)
