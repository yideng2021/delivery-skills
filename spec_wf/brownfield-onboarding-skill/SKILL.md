...
description: >
  ...
  并保留活查询出口,为 spec-wf 的 proposal §0 既有资产盘点与 design §5 复用决策提供准确的现状认知。
...
# Brownfield Onboarding — 棕地项目认知基线

## 触发

用户表达「梳理 / 理解既有项目」「棕地认知」「为既有工程做规约前先摸清现状」「老项目业务/架构梳理」,
或 spec-design-workflow 在 `change_mode != greenfield` 且缺现状认知时路由进入。
...
- (spec-wf 衔接时)proposal 的 `change_mode` / 关联意图
...
| `05-retrieval-guide.md` | 活查询指南 + spec-wf 对接映射 | — | [templates/05-retrieval-guide.md](templates/05-retrieval-guide.md) |
...
- **B5 指南** ← 套用 `05` 模板,实例化工具映射 + spec-wf 对接 → `05`
...
## 与 spec-wf 衔接(零命令耦合,被动消费)

本 skill **不进入** proposal→spec→design→tasks 4 步链,而是被 writer 在 proposal §0 / design §5 / tasks
拆分等节点**主动查询**(仿 RBK)。衔接细则与消费点查询表详见 [`references/spec-wf-integration.md`](references/spec-wf-integration.md)。
...
- 工具对比与原理:[`../../framework_git/docs/comparison_report.md`](../../framework_git/docs/comparison_report.md)
