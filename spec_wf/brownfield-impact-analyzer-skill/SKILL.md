...
双源输出是**线索**,可能漏报(反射/框架包装)或误报(死代码边)。校验**不是再查一次图**(同源自证无效),
而是**读 verbatim 源码核对**。校验深度按**工具自身输出**定级——工具先跑(廉价、全覆盖),其输出即风险分类器:

| 级别 | 工具信号(满足即归此级) | 校验动作 | impact 标注 |
|------|------------------------|----------|------------|
| **低** | blast radius 小 ∧ 不触达 public API ∧ 不跨域 ∧ 纯增量(不改既有路径) | 信任工具,不深读源码 | `☐ 仅工具` |
| **中** | 改既有逻辑 ∨ 跨域调用 ∨ 接口向后兼容扩展 | 对**关键边** `codegraph_explore` 读源码核对调用真伪与语义 | `✅ 源码已核` |
| **高** | 改业务流程 ∨ 状态机 ∨ 数据模型 ∨ 破坏性接口变更 | 源码核对 + 调 `.brownfield/` 取业务语义 + **❗人工确认** | `❗ 待人工确认` |

**校验地基序**:`verbatim 源码(实时)` > `活查询图(实时)` > `架构文档(pin 旧 commit)`。
- 架构文档只供**业务语义**(属哪个域、业务意图),**不裁判结构**——它比工具更易过时,结构冲突一律信源码。
- **共同盲区**:反射目标 / 运行时配置驱动分支 / 未显式编码的业务铁律——工具看不见,AI 读源码也看不见,
  **只有高层级的人工确认能兜底**。这是人工确认不可省的认识论原因,非流程冗余。
- **偏差留痕**:校验发现工具漏/误报时,在 §3 记一行 `⚠️ 工具漏报/误报:<edge> 经源码确认 …`,使校验可审计。
...
- [ ] §3 每行标了校验状态(`☐/✅/❗`);中/高级影响**已读 verbatim 源码核对**,未读源码不得直接落工具结论
