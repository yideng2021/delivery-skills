...
双源输出是**待校验线索**,可能漏报(反射/框架包装的调用)或误报(死代码边)。落 §3 前按 SKILL「分级校验协议」核对:

1. **地基是 verbatim 源码,不是再查一次图**。校验 = 对关键边跑 `codegraph_explore` 读真实源码,确认调用真实存在、语义如图;**重查 `codegraph_impact` 不算校验**(同源自证)。
2. **校验深度按工具输出分级**:低(仅工具,纯增量)/ 中(关键边读源码)/ 高(源码 + 业务语义 + 人工确认)。判级见 §2.2 风险等级。
3. **架构文档(`.brownfield/`)只供业务语义,不裁判结构**——它 pin 旧 commit,比工具更易过时,结构冲突一律信源码。
4. **偏差留痕**:校验改写了工具结论时,在 §3 对应行加 `⚠️ 工具漏报/误报:<edge> 经源码确认 …`,使结论可审计。
5. **共同盲区交人工**:反射目标 / 运行时配置分支 / 未显式编码的业务铁律,工具与 AI 读源码都看不见,高风险时显式标 `❗ 待人工确认`,不替用户拍板。
...
- ❌ **把工具输出直接当结论落 §3**(未读源码核对关键边,违反 §2.5 校验纪律)
- ❌ **用 `.brownfield/` 架构文档纠正工具的调用关系**(文档只供语义;结构信源码)
