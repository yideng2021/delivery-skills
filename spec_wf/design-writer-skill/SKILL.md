...
12. **reviewed 由用户裁决**:skill **不得**自行把 `status` 从 `draft` 升 `reviewed`;CDR 退出 + checklist 全勾选后必须以 ToolCall 三选项形态请用户裁决:`[1] 满意,升 reviewed` / `[2] 还需修改` / `[3] 重写 design`,得到 `[1]` 才执行 frontmatter 字段升级。本约束与 §3 L3 升级 ToolCall 门**正交**(后者只决定建模深度,不替代 reviewed 裁决)。
...
> 棕地改动的核心落点正是**技术设计**:当 `reused_modules` 含 `[已有·修改]` / `[已有·废弃]`,或某条 ADR 涉及对既有 BC / 模块的**重构、替换**时,建议调用 [`brownfield-impact-analyzer`](../brownfield-impact-analyzer-skill/SKILL.md) 产出咨询件 `impact.md`;其 §4 侵入与接缝建议 / §5 低耦合设计规则(Expand-Contract / 防腐层(ACL) / Branch by Abstraction / Façade / DIP 等**通用原则**)可作为 §5 ADR「复用充分性三问」与 §3 模块契约 / §3.2 依赖图的**设计输入素材**(被动引用,不强耦合)。`impact.md` 为**诊断咨询件**:只给原则参考,**具体落地设计仍由本 design 完成**;它**不进** 主 schema 校验,**不修改** 本 design 正文与 frontmatter,亦**不**触碰边界红线(原则名非 SQL/HTTP/代码片段)。
