...
> 本 skill 落锚阶段的检索口径。基于 CodeGraph + GitNexus + IDE 内置搜索的协同策略，
> 适用于 Java 后端 + Vue 前端多仓单工作区。
...
多仓单工作区场景下，必须通过参数指定目标仓库才能精准命中：

**CodeGraph** — 通过 `projectPath` 指定项目绝对路径：

```javascript
codegraph_explore({ query: "目标符号", projectPath: "<前端|后端绝对路径>" })
codegraph_callers({ symbol: "handler", projectPath: "<后端绝对路径>" })
```

**GitNexus** — 通过 `repo` 指定仓库名：

```javascript
gitnexus_query({ query: "业务流程", repo: "<仓库名>" })
gitnexus_context({ name: "ServiceImpl", repo: "<仓库名>" })
```

> 不指定时工具可能搜索错误仓库或返回空结果。**每次调用都显式指定目标**。
> 前端查前端索引、后端查后端索引——不混用。
...
```
接口 path
  → gitnexus route_map(route="/x", repo=<后端>) 或 codegraph_search(kind=route)
  → handler
...
```
接口 path
  → grep src/api/*.js → 前端 API 函数（读 api 定义确认实际 path）
  → codegraph_callers(API函数, projectPath=<前端>) → 调用组件
路由/文案
  → grep src/router、i18n → 页面组件
```
...
| 工具 | 一句话 | 关键参数 |
|------|--------|----------|
| `codegraph_explore` | **首选**。传入符号名/文件名，一次返回多文件源码+关系 | `query`, `maxFiles`(默认12), `projectPath` |
| `codegraph_search` | 模糊定位符号位置（不含源码） | `query`, `kind`(function/class/component/route等), `projectPath` |
| `codegraph_node` | 获取单个符号的完整源码，支持重载消歧 | `symbol`, `includeCode=true`, `file`, `line`, `projectPath` |
| `codegraph_callers` | 谁调用了这个符号 | `symbol`, `limit`(默认20), `projectPath` |
| `codegraph_callees` | 这个符号调用了谁 | `symbol`, `limit`, `projectPath` |
| `codegraph_impact` | 轻量影响面分析（按深度分组） | `symbol`, `depth`(默认2), `projectPath` |
| `codegraph_files` | 索引文件树，含语言/符号数统计 | `path`, `pattern`, `format`(tree/flat/grouped), `projectPath` |
...
| `rename` | 跨文件安全重命名，带 graph/text_search 置信度 | `symbol_name`, `new_name`, `dry_run`(默认true), `repo` |
...
- `codegraph_explore` 是最高效的单次调用工具，一次返回多文件源码+关系，**落锚首选**
- `gitnexus context` 比逐个 `callers/callees` 更全面（含 imports/extends/implements）
- 多仓场景下 **所有工具调用必须指定** `projectPath`（CodeGraph）或 `repo`（GitNexus）
- 读取大文件时用 `line_range` 精确定位，避免超限
- 搜索不到时**切换工具**而非反复调整同一工具的参数
